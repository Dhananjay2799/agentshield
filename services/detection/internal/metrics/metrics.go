...
type Snapshot struct {
	...
	ContainmentEvents           int64      `json:"containment_events"`
...
func (m *Metrics) RecordContainmentEvent() {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Metrics) Snapshot() Snapshot {
	...
	return Snapshot{
		...
		ContainmentEvents:           m.containmentEvents,
...
func (m *Metrics) WritePrometheus(w io.Writer) error {
	...
	metrics := []struct {
		...
		{
			"Total number of AgentShield containment control-plane events observed by detection.",
			"agentshield_detection_containment_events_total",
			"counter",
